...
aws s3 sync "$HERE/corpus" "s3://$BUCKET/corpus/" --profile "$PROFILE" --region "$REGION"
...
if [ "$KB_ID" = "None" ] || [ -z "$KB_ID" ]; then
  ...
  KB_ID="$(aws bedrock-agent create-knowledge-base --profile "$PROFILE" --region "$REGION" \
...
for i in $(seq 1 24); do
  KB_STATUS="$(aws bedrock-agent get-knowledge-base --knowledge-base-id "$KB_ID" \
    --profile "$PROFILE" --region "$REGION" --query "knowledgeBase.status" --output text 2>/dev/null || echo UNKNOWN)"
...
# 4. S3 data source ------------------------------------------------------------
DS_ID="$(aws bedrock-agent list-data-sources --knowledge-base-id "$KB_ID" --profile "$PROFILE" --region "$REGION" \
...
if [ "$DS_ID" = "None" ] || [ -z "$DS_ID" ]; then
  ...
  DS_ID="$(aws bedrock-agent create-data-source --profile "$PROFILE" --region "$REGION" \
...
for i in $(seq 1 36); do
  DS_STATUS="$(aws bedrock-agent get-data-source --knowledge-base-id "$KB_ID" --data-source-id "$DS_ID" \
    --profile "$PROFILE" --region "$REGION" --query "dataSource.status" --output text 2>/dev/null || echo UNKNOWN)"
...
aws bedrock-agent start-ingestion-job --profile "$PROFILE" --region "$REGION" \
...
============================================================
  ...
  Ingestion runs async (managed KB: ~2-5 min). Check with:
    aws bedrock-agent list-ingestion-jobs --knowledge-base-id $KB_ID --data-source-id $DS_ID --profile $PROFILE --region $REGION
============================================================
EOF
